...
import hamButton from '../assets/hamButton.svg';
...
: Post & { onEditClick: (postId: number) => void } & { onDeleteClick: (postId: number) => void } & { onLoad: () => void }) {
        ...
        <div className="bg-white p-4 rounded-lg border w-full lg:p-[30px]" style={{ boxShadow: '0px 0px 4px 0px rgba(0, 0, 0, 0.15)' }}>
            <div className="flex justify-between items-center mb-[10px] lg:mb-5">
                <div className="flex items-center">
                    <Link to={`/${username}`}>
                        <PhotoTweeks image={userPhoto} alt="User" backUp={defaultProfile} className="w-8 h-8 rounded-full object-cover lg:w-[74px] lg:h-[74px]" />
                    </Link>
                ...
                {(Number(activeUserId) === userId) &&
                    <button onClick={toggleSettingsMenu} className="relative self-start">
                        <img src={hamButton} className="h-[13px] mr-[1.5px] lg:h-[23px] lg:mr-1" />
                        {postSettings &&
                            <div className="absolute -top-[5.5px] right-[7px] bg-white w-[82px] h-[68px] flex flex-col justify-center font-medium rounded-lg shadow-[0px_4px_21px_-4px_rgba(0,0,0,0.2)] lg:right-[14px] lg:w-[105px] lg:h-[90px]">
            ...
            <PhotoTweeks image={photo_link} alt="Post" backUp={postWithoutPhoto} className="max-h-60 max-w-full object-cover rounded-[8.5px] mb-2 mx-auto lg:max-h-none lg:mb-6" onLoad={onLoad} />
            <div className="flex items-center">
                <LikeButton counter={likes} postId={id} />
            </div>
        </div>
    );
